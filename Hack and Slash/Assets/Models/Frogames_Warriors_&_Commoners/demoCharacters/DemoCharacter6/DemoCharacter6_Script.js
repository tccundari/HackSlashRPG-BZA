...
function Start()
{
	...
	animation["2h_run"].speed = ForwardSpeed/3;
...
function Update () 
{
    ...
    if(charController.isGrounded == true)
    {
		...
		//Idle
		} else if (Input.GetButton ("Jump")) animation.Play("2h_attack2");
		
		//Idle
		else {
			
			if(Input.GetAxis("Horizontal") && !Input.GetAxis("Vertical")) animation.CrossFade("turn");
			else animation.CrossFade("2h_idle", 0.1);		
